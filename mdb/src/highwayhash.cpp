...
void highwayHash::HighwayHashReset(const uint64_t key[4], HighwayHashState* state) {
...
void highwayHash::ZipperMergeAndAdd(const uint64_t v1, const uint64_t v0,
...
void highwayHash::Update(const uint64_t lanes[4], HighwayHashState* state) {
  int i;
  for (i = 0; i < 4; ++i) {
...
uint64_t highwayHash::Read64(const uint8_t* src) {
...
void highwayHash::HighwayHashUpdatePacket(const uint8_t* packet, HighwayHashState* state) {
  uint64_t lanes[4];
...
void highwayHash::Rotate32By(uint64_t count, uint64_t lanes[4]) {
  int i;
  for (i = 0; i < 4; ++i) {
...
void highwayHash::HighwayHashUpdateRemainder(const uint8_t* bytes, const size_t size_mod32, HighwayHashState* state) {
  int i;
...
void highwayHash::Permute(const uint64_t v[4], uint64_t* permuted) {
...
void highwayHash::PermuteAndUpdate(HighwayHashState* state) {
  uint64_t permuted[4];
  Permute(state->v0, permuted);
  Update(permuted, state);
}

uint64_t highwayHash::HighwayHashFinalize64(HighwayHashState* state) {
  int i;
  for (i = 0; i < 4; i++) {
...
void highwayHash::ProcessAll(const uint8_t* data, size_t size, const uint64_t key[4], HighwayHashState* state) {
  size_t i;
...
uint64_t highwayHash::operator()(const long long& data)const {
  pair<uint8_t*, int>Pair = hashKey(data);
  HighwayHashState state;
  const uint64_t key[4] = { 1,2,3,4 };
  ProcessAll(Pair.first, Pair.second, key, &state);
  return HighwayHashFinalize64(&state);
}

uint64_t highwayHash::operator()(string const& data)const {
  pair<uint8_t*, int>Pair = hashKey(data);
  HighwayHashState state;
  const uint64_t key[4] = { 1,2,3,4 };
  ProcessAll(Pair.first, Pair.second, key, &state);
